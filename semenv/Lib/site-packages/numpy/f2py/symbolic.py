    ...
    def polynomial_atoms(self):
        ...
        def visit(expr, found=found):
            if expr.op is Op.FACTORS:
                for b in expr.rawData:
                    b.traverse(visit)
                return expr
            if expr.op in (Op.TERMS, Op.COMPLEX):
                return
            if expr.op is Op.APPLY and isinstance(expr.rawData[0], ArithOp):
                if expr.rawData[0] is ArithOp.POW:
                    expr.rawData[1][0].traverse(visit)
                    return expr
                return
...
def normalize(obj):
    ...
    if obj.op is Op.APPLY and obj.data[0] is ArithOp.DIV:
        ...
        if t1.op is Op.APPLY and t1.rawData[0] is ArithOp.DIV:
            numer = t1.rawData[1][0] * c1
            denom = t1.rawData[1][1] * t2 * c2
            return as_apply(ArithOp.DIV, numer, denom)

        if t2.op is Op.APPLY and t2.rawData[0] is ArithOp.DIV:
            numer = t2.rawData[1][1] * t1 * c1
            denom = t2.rawData[1][0] * c2
            return as_apply(ArithOp.DIV, numer, denom)
    ...
    if obj.op is Op.TERNARY:
        ...
        if cond.op is Op.INTEGER:
            return expr1 if cond.rawData[0] else expr2
